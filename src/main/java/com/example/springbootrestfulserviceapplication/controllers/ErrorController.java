...
import com.example.springbootrestfulserviceapplication.exceptions.*;
import org.slf4j.*;
...
public class ErrorController {
    ...
    @ResponseBody
    public ResponseEntity<ErrorInfo> handleProductNotFoundException(UserNotFoundException e) {
    ...
    @ResponseBody
    public ResponseEntity<ErrorInfo> handleUserCreationException(UserCreationException e) {
        logger.error("Error creating User", e);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ErrorInfo("Error creating user: " + e.getMessage()));
    }
}
